...
describe('sitemapPaths', () => {
  ...
  it('includes the home page and the static public pages', () => {
    for (const p of ['/', '/about', '/faqs', '/blog', '/contact', '/privacy-policy']) {
  ...
  it('covers every curated place, and both gender segments of each', () => {
    for (const place of PLACES) {
      expect(paths).toContain(`/hostels/${place.slug}`);
      expect(paths).toContain(`/hostels/${place.slug}/girls`);
      expect(paths).toContain(`/hostels/${place.slug}/boys`);
    }
  });
  ...
  it('lists the landing pages that a careless /host disallow would block', () => {
    expect(paths.some((p) => p.startsWith('/hostels/'))).toBe(true);
  });
